...
Instagram Live Recorder 是一個用於錄製 Instagram 直播的工具，並通過指定的參數進行配置。本專案使用 Spring Boot 框架開發，並且支援使用 ffmpeg 處理錄製過程。
...
2. 不須設定只需執行 InstagramLiveRecorder.jar 來查看使用說明：
    ```bash
...
| `-interval` | 設置請求間隔（毫秒），範圍在 1 到 5000 之間，預設值為 300             | |
| `-history` | 是否需要記錄歷史資料                                      | | 
| `--help` | 顯示指令的使用說明                                       | |
